...
B4.1 and B4.2 contain only `InventoryState.AVAILABLE`. A Variant without history implicitly has available quantity zero and needs no balance row; the first movement or hold lazily creates the row. B4.2 models temporary reservations as separate `StockHold` records rather than a HELD balance state.
...
- `GET /api/v1/merchants/:merchantId/inventory/:variantId` (`inventory.read`) returns identity, physical `availableQuantity`, effective `heldQuantity`, derived `sellableQuantity`, and nullable `inventoryUpdatedAt`.
...
New writes verify the tenant-scoped Variant, insert the AVAILABLE projection with `ON CONFLICT DO NOTHING`, lock it with PostgreSQL `FOR UPDATE`, derive and validate the next quantity against physical stock and effective active holds, insert history, and update projection in one transaction. Database uniqueness resolves concurrent key races. PostgreSQL is authoritative; there is no Redis/in-memory stock cache or process lock.

The inventory ledger is stock-domain history, not a generic audit log or outbox. B4.2 adds StockHold reservations and effective expiry without a HELD state or ledger movement. B6.2 confirmation and confirmed cancellation create/release/expire Holds only: physical AVAILABLE and the ledger remain unchanged. See [STOCK_HOLDS.md](STOCK_HOLDS.md) and [ORDER_CONFIRMATION_STOCK_HOLDS.md](ORDER_CONFIRMATION_STOCK_HOLDS.md). Sale/fulfilment movement, warehouse/bin, transfer, return inspection, and stock caching remain deferred.
