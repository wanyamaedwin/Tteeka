...
B1.3 seeds neither default Roles nor a Permission catalog. B2.1 defines the first Merchant business keys. B2.2 adds a code-owned application catalog and explicit synchronization command without startup side effects or default Roles. Commerce modules such as B3.1 Catalogue define canonical Permission constants that feed that global code-owned catalog without creating runtime Nest module dependencies.
