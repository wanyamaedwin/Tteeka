...
B1.8 exposes context and a pure permission evaluator. It does not enforce a permission on this endpoint, add permission decorators or guards, seed authorization data, or add role-management APIs.
...
There are no wildcard semantics, deny rules, Role-name shortcuts, hierarchy, implication, or prefix matching. A key such as `order.*` is an ordinary literal key if it exists. Permission enforcement belongs to a later checkpoint; B1.8 only provides deterministic context and evaluation primitives.

## Deferred work

- permission requirement decorators and a permission-enforcement guard
