...
# B8.1 delivery permissions

`deliveries.read` protects DeliveryJob list/detail and DeliveryAttempt history. `deliveries.manage` protects creation, ready, dispatch, cancel, and attempt recording. They are exact independent grants: manage does not expose reads and read cannot mutate operational state.

Delivery permissions are independent from Orders and Payments permissions. A caller with the appropriate Delivery grant may operate on a known Order or Delivery ID without `orders.read`, `orders.manage`, `payments.read`, or `payments.manage`; the Delivery store performs tenant-scoped internal validation. The explicit catalog now has 21 keys. Synchronization remains explicit and idempotent, runs neither at startup nor automatically, and creates no Role grants. See [DELIVERY_JOBS_ATTEMPTS.md](DELIVERY_JOBS_ATTEMPTS.md).
